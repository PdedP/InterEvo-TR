...
public class MOSATestSuiteAdapter extends TestSuiteAdapter<AbstractMOSA> {
    ...
    public List<TestSuiteChromosome> getBestIndividuals() {
    	...
    	List<TestChromosome> preferredtestCases = new ArrayList<>();
    	for(TestChromosome test : PreferencesArchive.getArchiveInstance().getSolutions()) {
    		preferredtestCases.add(test);
		}
    	
    	List<TestChromosome> orderedByReadability = preferredtestCases.stream().sorted(
    			Comparator.comparing(TestChromosome::getReadabilityValue)).collect(Collectors.toList());
    	
    	for (TestChromosome test : orderedByReadability) {
    		test.setPreferred(true);
    		newTestSuite.addTest(test);
        }
    	
    	TestSuiteChromosome bestTestCases = Archive.getArchiveInstance().mergeArchiveAndSolution(newTestSuite);
