...
public class DynaMOSA extends AbstractMOSA {
    ...
    protected void evolve() {
        ...
        Archive.iteration = this.currentIteration;	// ADDED
    ...
    public void generateSolution() {
        ...
        /* ADDED */ 
        if (Properties.ENABLE_SECONDARY_OBJECTIVE_AFTER > 0 || Properties.REVISE_AFTER_PERCENTAGE_OF_COVERAGE > 0) {
        ...
        while (!isFinished() && this.goalsManager.getUncoveredGoals().size() > 0) {
            ...
            /* ADDED */
            updateSecondaryCriterion(0, this.goalsManager, this.currentIteration);
            
            if(Properties.INTERACTIVE_PROCESS && TestChromosome.isFirstSecondaryObjectiveEnabled()) { 
            	
                TestChromosome.disableFirstSecondaryObjective();	//From now on, only enable if the preference archive has to be updated 
                
                if(Properties.MAX_TIMES_TO_SORT_REVISE > this.goalsManager.getNumberSortInteractions()){
                	
                    Properties.TEST_ARCHIVE = false;
                    
                    InteractiveEvaluationSecondaryObjective.initializeIData();
                    InteractiveEvaluationSecondaryObjective.getIData().setIteration(currentIteration);
                    
                    //Start counting time for interaction
                    long startInteractionTime = System.currentTimeMillis();
					
                    this.goalsManager.executeInteraction(this.population);
					
                    //Finish counting time for interaction
                    interactionTimeTotal += (System.currentTimeMillis() - startInteractionTime);
					
                    //The current state of the archive of preferences is shown to the tester 
                    this.goalsManager.showArchivePreferences();
        ...
        if(Properties.INTERACTIVE_PROCESS && Properties.INTERACTION_STATISTICS) {
            InteractiveEvaluationSecondaryObjective.writeInteractions(interactionTimeTotal);
			
            if(Properties.EXECUTION_NUMBER >= 0) {
                //Show the number of different goals interacted in a file
                this.goalsManager.writeGoalsInteracted();
            }
        }
        
        /* ADDED: for experiments on test case length */
        if (!Properties.INTERACTION_STATISTICS && Properties.STOPPING_CONDITION == StoppingCondition.MAXGENERATIONS) {
            this.goalsManager.writeArchiveMinimized();
        }

        this.notifySearchFinished();
        
        LoggingUtils.getEvoLogger().info("\n\n--- EXECUTION FINISHED ---\n_________________________\n\n");
        LoggingUtils.getEvoLogger().info("Generating the final test suite...\n");
        ps.println("\n\n--- EXECUTION FINISHED ---\n_________________________\n");
        ps.println("Generating the final test suite...");
    }
