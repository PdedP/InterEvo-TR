...
import org.evosuite.ga.archive.PreferencesArchive;
    ...
    protected List<TestChromosome> breedNextGeneration() {
        ...
        for (int i = 0; i < Properties.POPULATION * Properties.P_TEST_INSERTION; i++) {
            ...
            if (this.getCoveredGoals().size() == 0 || Randomness.nextBoolean()) {
            	...
            	//ADDED: Add the possibility to select the preference archive as source
            	//This happens when the preferences archive is not empty and with probability P_PREFERENCE_SELECTION
            	if(this.getPreferences().isEmpty() || Randomness.nextDouble() > Properties.P_PREFERENCE_SELECTION) {
            		tch = Randomness.choice(this.getSolutions()).clone();
            	}
            	else {
            		tch = Randomness.choice(this.getPreferences()).clone();
            		tch.getTestCase().removeAssertions();					//Remove assertions
            	}
...
//				tch.mutate(); // TODO why is it mutated twice?
    ...
    /*ADDED*/
    /**
     * Return the test cases in the preference archive as a list.
     *
     * @return
     */
    protected List<TestChromosome> getPreferences() {
    	return new ArrayList<>(PreferencesArchive.getArchiveInstance().getSolutions());
    }
