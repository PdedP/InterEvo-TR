...
public abstract class GeneticAlgorithm<T extends Chromosome<T>> implements SearchAlgorithm,
    ...
    boolean objectives = false;	//For experiments
    ...
    protected void disableFirstSecondaryCriterion() {     
        if (TestSuiteChromosome.getSecondaryObjectivesSize() > 1) {
            ...
            /* ADDED */
            if(Properties.INTERACTIVE_PROCESS && Properties.DISABLE_SECOND_SECONDARY_OBJECTIVE) {
            	TestSuiteChromosome.disableSecondSecondaryObjective();
            }
            else{
    ...
    protected void enableFirstSecondaryCriterion() {
        if (TestSuiteChromosome.getSecondaryObjectivesSize() > 1) {
            ...
            /* ADDED */
        	if(Properties.INTERACTIVE_PROCESS) {
            	if(Properties.DISABLE_SECOND_SECONDARY_OBJECTIVE) {
            		TestSuiteChromosome.enableSecondSecondaryObjective();
            	}
            }
  ...
  //ADDED
    protected void disableFirstSecondaryCriterionTestChromosome() {     
        if (TestChromosome.getSecondaryObjectivesSize() > 1) {
        	TestChromosome.disableFirstSecondaryObjective();
            
            /* ADDED */
            if(Properties.INTERACTIVE_PROCESS && Properties.DISABLE_SECOND_SECONDARY_OBJECTIVE) {
            	TestChromosome.disableSecondSecondaryObjective();
            }
            else{
            	if (ArrayUtil.contains(Properties.SECONDARY_OBJECTIVE, Properties.SecondaryObjective.IBRANCH)) {
            		ExecutionTracer.disableContext();
    ...
    //ADDED
    protected void enableFirstSecondaryCriterionTestChromosome() {
        if (TestChromosome.getSecondaryObjectivesSize() > 1) {
            
            /* ADDED */
        	if(Properties.INTERACTIVE_PROCESS) {
            	if(Properties.DISABLE_SECOND_SECONDARY_OBJECTIVE) {
            		TestChromosome.enableSecondSecondaryObjective();
            	}
            }
     ...
     * @param goals
     */
    protected void updateSecondaryCriterion(int starvationCounter, MultiCriteriaManager goals, int progress) {

        if (Properties.ENABLE_SECONDARY_OBJECTIVE_AFTER > 0
                && TestChromosome.getSecondaryObjectivesSize() > 1) {

            if (progress > Properties.ENABLE_SECONDARY_OBJECTIVE_AFTER) {
                 	
                if(Properties.REVISE_AFTER_PERCENTAGE_OF_COVERAGE > 0 && Properties.INTERACTIVE_PROCESS) {
                	updateSecondaryObjectiveCoverage(goals, progress);
                } else {
                    enableFirstSecondaryCriterionTestChromosome();
                    
                    /* ADDED */
                    if(Properties.INTERACTIVE_PROCESS)
                    	updateSecondaryObjectiveInteractivity(progress);
                    else
                    	Properties.ENABLE_SECONDARY_OBJECTIVE_AFTER = 0;
                }
            }
            /* //ADDED 
            	...
            	disableFirstSecondaryCriterionTestChromosome();
            }*/
        } 
        /* ADDED */
        else if(Properties.REVISE_AFTER_PERCENTAGE_OF_COVERAGE > 0) {	//For experiments
        		// && Properties.ENABLE_SECONDARY_OBJECTIVE_AFTER == 0 
        		//&& Properties.INTERACTIVE_PROCESS 
        		//&& TestChromosome.getSecondaryObjectivesSize() > 1) {              	
        	updateSecondaryObjectiveCoverage(goals, progress);
        }
    }
    ...
    /* ADDED */
    private void updateSecondaryObjectiveInteractivity(int progress) {
    	//If the user wants to interact periodically, we add the indicated frequency to the current progress.
    	//Otherwise, ENABLE_SECONDARY_OBJECTIVE_AFTER = 0 and the secondary objective is always enabled after that.
    	if(Properties.REVISE_PERIODICALLY) {
    	
    		//ADDED
    		if(Properties.FREQUENCY_STRATEGY == Properties.FrequencyStrategy.PROGRESSIVE) {
    			//Update frequency
    			Properties.FREQUENCY = Properties.FREQUENCY * Properties.FREQUENCY_SCALE;
    		}
    		//
    		
    		Properties.ENABLE_SECONDARY_OBJECTIVE_AFTER = progress + Properties.FREQUENCY - 1;
    	
    		int l = limitIterations();
    		if(Properties.ENABLE_SECONDARY_OBJECTIVE_AFTER >= l - 1) {
    			Properties.ENABLE_SECONDARY_OBJECTIVE_AFTER = l - 2;
    		}
    	}
    	else {
    		Properties.ENABLE_SECONDARY_OBJECTIVE_AFTER = 0;
    ...
    /* ADDED */
    private void updateSecondaryObjectiveCoverage(MultiCriteriaManager goals, int progress) {
    	int totalGoals = goals.getUncoveredGoals().size() + goals.getCoveredGoals().size();
    	double coverage = (goals.getCoveredGoals().size() * 100/ (double)totalGoals);
		
		if(coverage > Properties.REVISE_AFTER_PERCENTAGE_OF_COVERAGE) {
			
			if(Properties.INTERACTIVE_PROCESS) {		
				enableFirstSecondaryCriterionTestChromosome();
	
				//After this, REVISE_AFTER_PERCENTAGE_OF_COVERAGE is not used anymore. 
				//We transfer the control to ENABLE_SECONDARY_OBJECTIVE_AFTER
				Properties.REVISE_AFTER_PERCENTAGE_OF_COVERAGE = 0;
				
	        	updateSecondaryObjectiveInteractivity(progress);
			}
			
			// ADDED: For experiments
			if(!Properties.INTERACTION_STATISTICS && !objectives) {
					
				PrintStream ps = null; 
				objectives= true;
				try {					
					ps = new PrintStream(new File(Properties.CLASS_TYPE_DIR + "/list-goals-" + Properties.EXECUTION_NUMBER + ".txt"));
				} catch (FileNotFoundException e) {
					e.printStackTrace();
				}
				
				ps.println("COVERED: " + goals.getCoveredGoals().size());
				ps.println("========================================================");
				for (FitnessFunction<TestChromosome> ff : goals.getCoveredGoals()){
					ps.println(ff.getClass().getSimpleName());	
				}
				
				ps.println("\nUNCOVERED: " + goals.getUncoveredGoals().size());
				ps.println("========================================================");
				for (FitnessFunction<TestChromosome> ff : goals.getUncoveredGoals()){
					ps.println(ff.getClass().getSimpleName() + " - " + ff.toString());	
				}
				ps.println("\nCURRENT: " + goals.getCurrentGoals().size());
				ps.println("========================================================");
				for (FitnessFunction<TestChromosome> ff : goals.getCurrentGoals()){
					ps.println(ff.getClass().getSimpleName() + " - " + ff.toString());	
				}
    ...
    //ADDED
    /**
     * Returns the available iterations for the search
     *
     * @return value of getLimit() in MaxGenerationStoppingCondition
     */       
    protected int limitIterations() {
    	
    	int limit = 0;
        for (StoppingCondition<T> sc : this.stoppingConditions) {
        	if(sc instanceof MaxGenerationStoppingCondition<?>) 
    			limit = (int) sc.getLimit();
        }
