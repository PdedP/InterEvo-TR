...
public abstract class Archive implements Serializable {
    ...
    public Map<TestFitnessFunction, ArchiveData> whenAddedToArchive = new HashMap<TestFitnessFunction, ArchiveData>();
    public List<TestFitnessFunction> newlyAdded = new ArrayList<>();
    ...
    public boolean isBetterThanCurrent(TestChromosome currentSolution, TestChromosome candidateSolution) {
        ...
        /* OLD VERSION: This version is not suitable for more than one secondary objective as it requires that both objectives are better 
         ...
         *    // If we try to add a test for a target we've already covered
           ...
           //ADDED:
           /* NEW VERSION: It gives priority to the first secondary objective, and the second one is only evaluated when there is a tie in the first     */
           int result = 0;
           for (SecondaryObjective<TestChromosome> obj : getSecondaryObjectives()) {
           	result = obj.compareChromosomes(candidateSolution, currentSolution);
           	
           	//Three cases:
           	// If result < 0, the new solution is better => return true
               if (result < 0) {
               	return true;
               }
               // If result > 0, the new solution is worse => return false
               else if(result > 0) {
               	return false;
               }
               //If result == 0, delegate in the following secondary objectives
           }
