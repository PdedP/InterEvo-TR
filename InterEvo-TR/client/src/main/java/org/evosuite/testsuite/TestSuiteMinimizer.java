...
public class TestSuiteMinimizer {
    ...
    public void minimize(TestSuiteChromosome suite, boolean minimizePerTest) {
        ...
        //* ADDED */
        // since this method directly extract the first secondary objective, we ensure that the same behavior is preserved by reverting the strategy to the size.  
        if ( strategy  == SecondaryObjective.INTERACTIVE) {
        	strategy = SecondaryObjective.SIZE;
        }
    ...
    private void minimizeTests(TestSuiteChromosome suite) {
        ...
        /* ADDED: prioritize tests in preference archive */
        List<TestChromosome> alreadyMinimizedTests = new ArrayList<>();
        for (TestChromosome test : suite.getTestChromosomes()) {
        	if(test.isPreferred()) {
        		
                TestChromosome copy = test.clone();
                // Need proper list of covered goals
        		...
        		//	minimizedSuite.insertTest(copy.getTestCase());	// Delay the inclusion of these tests in the final test suite
        		alreadyMinimizedTests.add(copy);
        ...
        // ADDED: Tests in the preference archive are included at this point
        for (TestChromosome test : alreadyMinimizedTests) {
        	minimizedSuite.insertTest(test.getTestCase());
        }
        //
        ...
        // ADDED: preferred tests appear at the beginning
        Collections.reverse(suite.getTestChromosomes());
