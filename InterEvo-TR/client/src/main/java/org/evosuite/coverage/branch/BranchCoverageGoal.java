...
public class BranchCoverageGoal implements Serializable, Comparable<BranchCoverageGoal> {
    ...
    public String toStringInteractive() {
		...
		if (branch != null) {
			name += "Branch goal (" + branch.getActualBranchId() + "): \n";
		}
		else {
			name += "Branch goal: \n";
		}
				
		name += "Class: " + className.substring(className.lastIndexOf(".")+1) + "\n" + 
				"Method: " + methodName + "\n";
		
		 if (branch != null) {
				name += "Line: " + branch.getInstruction().getLineNumber() + "\n";
	            name += "Branch: " + branch.toStringInteractive();
	            if (value)
	                name += " - true";
	            else
	                name += " - false";
