...
//			test.addStatement(loadClassStmt);
    ...
    protected void postProcessTests(TestSuiteChromosome testSuite) {
		...
		/*ADDED*/ //For experiments with simulated interactivity
		if(Properties.EXECUTION_NUMBER >= 0) {
			
			if(Properties.SIMULATED_INTERACTIVITY) {
				PrintStream ps = null;
				try {					
					ps = new PrintStream(new File(Properties.CLASS_TYPE_DIR + File.separator + "tests" + Properties.EXECUTION_NUMBER + ".txt"));
				
					ps.println("Test suite: " + testSuite.getTests().size() + "\n"); //Show the test suite in the file
					int i = 0;
					for(TestChromosome tc : testSuite.getTestChromosomes()) {
						ExecutionResult result = tc.getLastExecutionResult();
						if (result == null) {
							result = TestCaseExecutor.runTest(tc.getTestCase());
						}
						ps.println("Test " + i++ + ":");
						ps.println(tc.getTestCase().toCode(result.getCopyOfExceptionMapping()));
					}
					
					ps.close();
					
					//This is for assessment of individual test cases instead of the entire test suite
					if (!Properties.INTERACTION_STATISTICS) {
					
						String directoryTestCases = Properties.CLASS_TYPE_DIR + File.separator + "testcases/";
						
						try {
							Files.createDirectories(Paths.get(directoryTestCases));
						} catch (IOException e) {
							e.printStackTrace();
						}
						...
						for(TestCase t : testSuite.getTests()) {
							...
							ps = new PrintStream(new File(directoryTestCases + "tests" + Properties.EXECUTION_NUMBER + "-" + counter + ".txt"));
							
							ps.println(t.toString());				//Show the test case in the file	   
							ps.close();
				...
				TestSuiteGenerator.writeJUnitTestsAndCreateResult(testSuite, "_ES_Test", true, Properties.CLASS_TYPE_DIR);
			}
		}
		//
    }
     ...
     */
	public static TestGenerationResult writeJUnitTestsAndCreateResult(TestSuiteChromosome testSuite, String suffix, boolean onlyToShow, String directory) {
        List<TestCase> tests = testSuite.getTests();
        
    	// ADDED
  		if (Properties.JUNIT_TESTS || onlyToShow) {
  			...
  			suiteWriter.writeTestSuite(name + suffix, testDir, testSuite.getLastExecutionResults(), onlyToShow);
  		}
  		return TestGenerationResultBuilder.buildSuccessResult();
    }
	
	public static TestGenerationResult writeJUnitTestsAndCreateResult(TestChromosome test, String suffix, boolean onlyToShow, String directory) {
		...
		if (Properties.JUNIT_TESTS || onlyToShow) {
			...
			List<ExecutionResult> execRes =new ArrayList<>();
			execRes.add(test.getLastExecutionResult());
			//ADDED
			suiteWriter.writeTestSuite(name + suffix, testDir, execRes, onlyToShow);
		}
		if(!onlyToShow)
			return TestGenerationResultBuilder.buildSuccessResult();
		return null;
    ...
    public static TestGenerationResult writeJUnitTestsAndCreateResult(TestSuiteChromosome testSuite) {
		//ADDED
		return writeJUnitTestsAndCreateResult(testSuite, Properties.JUNIT_SUFFIX, false, "");
    }
    ...
    public void writeJUnitFailingTests() {
        ...
        if (Properties.JUNIT_TESTS) {
            ...
            // ADDED
            suiteWriter.writeTestSuite(name + Properties.JUNIT_FAILED_SUFFIX, testDir, suite.getLastExecutionResults(), false);
        }
    }
